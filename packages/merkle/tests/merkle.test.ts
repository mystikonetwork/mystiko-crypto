...
import { createMerkleTreeWithWorker, MerkleTree } from '../src';
...
test('test constructor', () => {
  const tree1 = MerkleTree.fromLeaves();
  ...
  const tree2 = MerkleTree.fromLeaves([element1, element2]);
  ...
  const tree3 = MerkleTree.fromLeaves([], { maxLevels: 1, zeroElement: toBN(0) });
...
test('test path', () => {
  ...
  const tree = MerkleTree.fromLeaves([element1, element2, element3], { maxLevels: 2 });
...
test('test createMerkleTreeWithWorker', async () => {
  const tree1 = await createMerkleTreeWithWorker();
  expect(tree1.root().toString()).toBe(
    '17749238747541177922260023106539184144732198174810064796938596694265936155259',
  );
  const element1 = toBN('12d7aafbf3d4c1852ad3634d69607fc9ea8028f2d5724fcf3b917e71fd2dbff6', 16);
  const element2 = toBN('062c3655c709b4b58142b9b270f5a5b06b8df8921cbbb261a7729eae759e7ec3', 16);
  const tree2 = await createMerkleTreeWithWorker([element1, element2]);
  expect(tree2.root().toString()).toBe(
    '21205178834650720622262399337497375208854240907281368468056255721030220387133',
  );
});
