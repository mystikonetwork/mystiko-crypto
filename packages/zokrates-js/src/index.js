...
const initialize = async () => {
  ...
  const defaultProvider = {
    ...
    computeWitness: (input, args, computeOptions = {}) => {
      const { program, abi } = input instanceof Uint8Array ? { program: input, abi: null } : input;
  ...
  const withOptions = (options) => {
    return {
      ...
      universalSetup: (size, entropy) => defaultProvider.universalSetup(options.curve, size, entropy),
      setupWithSrs: (srs, program) => defaultProvider.setupWithSrs(srs, program, options),
      ...
      exportSolidityVerifier: (vk) => defaultProvider.exportSolidityVerifier(vk),
      utils: {
